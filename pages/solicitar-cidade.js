...
export default function SolicitarCidade() {
  const { user } = useAuth();
  ...
  if (!user) return <div className="text-center text-white mt-10">Faça login para solicitar papel de dono de cidade.</div>;
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-gamer">
      <form onSubmit={handleSolicitar} className="bg-glass p-8 rounded-xl shadow-2xl w-full max-w-md backdrop-blur-lg border border-primary">
        <h2 className="text-2xl font-gamer text-primary mb-6 text-center">Solicitar Papel de Dono de Cidade</h2>
